defmodule Membrane.RTP.AV1.SequenceDetector do
  ...
  of a coded video sequence. Per RFC 9628, this indicates a point from which
  decoding can begin.

  For encoders like SVT-AV1 (used by OBS) that use continuous intra refresh,
  every frame includes a sequence header. The decoder CAN start decoding from
  any frame that has a sequence header, so we set N=1 whenever a sequence
  header is present.
  """
  ...
  @doc """
  Checks if an access unit starts a new coded video sequence.

  Returns `true` if the access unit contains a sequence header OBU (type 1).
  Per RFC 9628, N=1 indicates a point from which decoding can begin,
  which is any frame with a sequence header.

  ## Examples

      iex> keyframe_with_seq_header = <<...>>  # Frame with sequence header
      iex> SequenceDetector.is_new_coded_video_sequence?(keyframe_with_seq_header)
      true

      iex> delta_frame = <<...>>  # Frame without sequence header
      iex> SequenceDetector.is_new_coded_video_sequence?(delta_frame)
      false
  """
  ...
  def is_new_coded_video_sequence?(access_unit) when is_binary(access_unit) do
    ...
    if has_sequence_header do
      require Logger
      Logger.debug("New coded video sequence detected (sequence header present)")
    end
